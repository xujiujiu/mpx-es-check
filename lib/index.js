...
function parseCode (options) {
  const { files, useAllRules } = options
  ...
  files.forEach(pattern => {
    ...
    const esListLogPath = options.output.replace(path.basename(options.output), 'es-list.log')
    const logger = createLogger(esListLogPath)
    const problemTypes = []
    globbedFiles.forEach(file => {
      ...
      problems.forEach((problem) => {
        if (!problemTypes.includes(problem.message)) problemTypes.push(problem.message)
      })
      if (
    ...
    if (problemTypes.length) logger.log(`${JSON.stringify(problemTypes, null, 2)}`)
  })
...
function check (file, code, options) {
  ...
  const configuredRules = collectRule(version, useAllRules, options, checkMiniprogram)
  ...
  if (!silent && problems.length) {
    ...
    logger.log(`${problemErrorInfo}`)
  } else {
    createLogger(options.output)
  }
  return problems
}
